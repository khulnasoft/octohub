import { constants, UserPlan } from '@octohub/core'
...
import { getDefaultOctoHubHeaders } from '../helpers'
    ...
    void (async () => {
      try {
        const response = await fetch(constants.GRAPHQL_ENDPOINT, {
          ...
          headers: {
            ...getDefaultOctoHubHeaders({ appToken: authData.appToken }),
      ...
      if (shouldPrompt) {
        reason = prompt(
          'Cancel subscription? Your subscription will be cancelled at the end of the billing period.' +
            ' You can keep using OctoHub for the already paid period.' +
    ...
    void (async () => {
      try {
        ...
        const response = await fetch(constants.GRAPHQL_ENDPOINT, {
          ...
          headers: {
            ...getDefaultOctoHubHeaders({ appToken }),
            'Content-Type': 'application/json',
          },
        })
