...
}): Record<keyof OctoHubHeaders, string> {
  const headers: Record<keyof OctoHubHeaders, string> = {
    Authorization: appToken ? `bearer ${appToken}` : '',
    OCTOHUB_HOSTNAME: constants.HOSTNAME || '',
    OCTOHUB_IS_BETA: `${!!constants.IS_BETA}`,
    OCTOHUB_IS_DEV: `${process.env.NODE_ENV !== 'production'}`,
    OCTOHUB_IS_LANDING: 'true',
    OCTOHUB_PLATFORM_IS_ELECTRON: 'false',
    OCTOHUB_PLATFORM_OS: getPlatform(),
    OCTOHUB_PLATFORM_REAL_OS: getOSName() || '',
