...
import { constants, tryParseOAuthParams } from '@octohub/core'
          ...
          },
          {
            ...
            onPress: () => {
              const description = Platform.isMacOS
                ? 'OctoHub (macOS native)'
                : Platform.isElectron
                ? `OctoHub (${Platform.realOS})`
                : Platform.isDesktop
                ? `OctoHub (${Platform.realOS})`
                : Platform.isPad
                ? 'OctoHub (iPad)'
    ...
    try {
      ...
      if (constants.LOCAL_ONLY_PERSONAL_ACCESS_TOKEN) {
        ...
        if (scope.length && !scope.includes('repo')) {
          throw new Error(
            'You didn\'t include the "repo" permission scope,' +
              ...
              " Your token will be safe on your device, and will never be sent to OctoHub's server.",
          )
        }
        ...
        const response = await axios.post(
          ...
          { token },
          { headers: getDefaultOctoHubHeaders({ appToken: existingAppToken }) },
        )
        setIsExecutingOAuth(false)
        setPATLoadingState(undefined)
