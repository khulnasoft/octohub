import { constants, Plan, PlanID } from '@octohub/core'
  ...
  */
  ...
  useEffect(() => {
    ...
    Browser.openURLOnNewTab(
      `${constants.OCTOHUB_LINKS.PRICING_PAGE}?appToken=${
        appTokenRef.current || ''
      }`,
    )
  }, [])
  ...
  const CancelOrReactivateSubscriptionButton = !!(
    ...
    <ButtonLink
      ...
      href={`${constants.OCTOHUB_LINKS.ACCOUNT_PAGE}?appToken=${appToken}`}
      openOnNewTab
      type="neutral"
    >
      Manage subscription ↗
    ...
    userPlan && userPlan.cancelAtPeriodEnd && userPlan.cancelAt ? (
      ...
      <Button
        ...
        onPress={() => {
          if (Platform.OS !== 'web') {
            Browser.openURLOnNewTab(
              `${constants.OCTOHUB_LINKS.ACCOUNT_PAGE}?appToken=${appToken}`,
            )
            return
          }
        ...
        {showUserPlanAtTheTop && !allowChangePlansOnThisApp ? (
          ...
          >
            <ButtonLink
              ...
              href={`${constants.OCTOHUB_LINKS.PRICING_PAGE}?appToken=${appToken}`}
              openOnNewTab
              type="neutral"
            >
              {userPlan &&
              ...
              <Button
                ...
                onPress={() => {
                  ...
                  if (Platform.OS !== 'web' || selectedPlan.type === 'team') {
                    Browser.openURLOnNewTab(
                      `${constants.OCTOHUB_LINKS.SUBSCRIBE_PAGE}?plan=${selectedPlan.cannonicalId}&appToken=${appToken}`,
                    )
                    return
                  }
